...
tooltips.removeIfExists = function() {
...
tooltips.generateHTMLFromData = function(postingData, tooltip, quoteUrl, isInline) {
  ...
  if (isInline) {
    posting.parseExistingPost(clone.getElementsByClassName('linkSelf')[0]);
  } else {
...
tooltips.loadTooltip = function(tooltip, quoteUrl, replyId, isInline) {
  ...
  if (tooltips.postCache[quoteUrl]) {
    ...
    if (board === api.boardUri && thread === api.threadId) {
      ...
      if (backContainer) {
        var tempBacklinks = temp.getElementsByClassName("panelBacklinks");
        Array.from(backContainer.container.children).forEach((backlink) => {
           Array.from(tempBacklinks).forEach((tempPanel) => {
             tempPanel.append(backlink.cloneNode(true));
           })
        });
      }
    }
	...
	tooltip.append(temp);
    if (isInline) {
      posting.parseExistingPost(temp.getElementsByClassName('linkSelf')[0], false, false, true);
    } else {
	  tooltips.checkHeight(tooltip);
    }
