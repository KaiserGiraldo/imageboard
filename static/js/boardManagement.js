...
boardManagement.setJs = function() {
  ...
  api.formApiRequest('setCustomJs', {
    ...
    if (status === 'ok') {
      ...
      if (file) {
        alert('新しいJavaScriptセット。');
      } else {
        alert('Javascriptが削除されました。');
      }
...
boardManagement.saveSettings = function() {
  ...
  var typedTags = document.getElementById('tagsField').value;
